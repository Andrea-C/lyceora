...
**Interfaces:**
- Consumes: `loadTaxonomy`, `buildGraph`, `prerequisiteClosure` (Tasks 2–3); anchor topic id list from `packages/taxonomy/data/math-it-media/anchors.json` (Task 5 — for this task, use the anchor id list given in Task 5's data; if executing before Task 5, create `anchors.json` first from that list).
...
Note: `generateObject` calls are exercised against a mocked model only in Task 16's eval script; unit tests here cover the pure logic. If the installed `ai` major exposes a different mock helper name than the eval script expects, adapt the import in the eval script only — not the production code.
...
- Produces:
  ...
  - `streamTeacher(reg: Registry, ctx: TeacherContext, messages: ModelMessage[]): Promise<{ textStream: AsyncIterable<string> }>`
...
export async function streamTeacher(
  ...
  return reg.withFallback("teacher", async ({ model }) => {
    const result = streamText({ model, system: buildTeacherSystemPrompt(ctx), messages });
...
beforeAll(async () => {
  ...
  await migrate(d, { migrationsFolder: new URL("../../../packages/db/drizzle", import.meta.url).pathname });
...
```ts
import { loadTaxonomy, buildGraph, assertAcyclic, type Topic, type TopicGraph } from "@lyceora/taxonomy";
...
import resources from "@lyceora/taxonomy/data/math-it-media/resources.json";
import paths from "@lyceora/taxonomy/data/math-it-media/paths.json";

export interface CuratedResource {
  id: string; topicIds: string[]; kind: "video" | "exercises" | "assessment";
  provider: string; title: { it: string; en: string }; url: string; lang: "it" | "en";
}
...
export function getResources(topicId: string): CuratedResource[] {
  return (resources as { resources: CuratedResource[] }).resources.filter((r) => r.topicIds.includes(topicId));
}
```
...
const fakeAssessor: AssessorPort = {
  ...
  grade: async (_ex, answer) => ({ correct: answer === "8", feedback: "ok" })
};
